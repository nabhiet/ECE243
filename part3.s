...
TIMER_CHECK:      CMP 	   R5, #72              // check for FPGA timer interrupt
				  BNE 	   KEYS_CHECK
				  
				  BL 	   TIMER_ISR
                  B 	   EXIT_IRQ

KEYS_CHECK:       CMP 	   R5, #73
...
TOGGLE:			  LDR      R4, =RUN
				  LDR      R3, [R4]
				  EOR      R3, #1               // toggle last bit 
				  STR      R3, [R4]
				  B        KEY_ISR_END

DOUBLE: 		  LDR      R1, =0xFF202000      // interval timer base address
				  ...
				  MOV      R3, #0x7             // START = 1, CONT = 1, ITO = 1
				  STR      R3, [R1, #0x4]
				  B        KEY_ISR_END
				  
HALF:			  LDR      R1, =0xFF202000      // interval timer base address
...
TIMER_ISR:        PUSH     {R0-R4, LR}
				  ...
				  LDR      R2, =COUNT       
				  LDR      R3, RUN              // get the run signal
				  LDR      R4, [R2]				// get the current count value
				  ADD      R4, R3               // increment by the value of the run signal
				  STR      R4, [R2]				// store the new count
				  
TIMER_ISR_END:    POP      {R0-R4, LR}
				  BX       LR
...
CONFIG_TIMER:                             
				  ...
				  LDR      R1, =25000000		// 1/(100 MHz) x 25x10^6 = 0.25 sec
