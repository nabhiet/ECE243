/* Program that displays a real time clock on HEX3-0 */
...
_start:     LDR     R4, =0xFF200020 // base address of HEX3-HEX0
			...
			MOV     R8, #0          // counter for HEX1-0 (initial value is 0)
			MOV     R11, #0         // counter for HEX3-2 (initial value is 0)
...
DISPLAY:	ADD     R8, #1			// start/continue counting for HEX1-0
			CMP     R8, #99
			ADDGT   R11, #1			// start/continue counting for HEX3-2
			CMP     R8, #99
			MOVGT   R8, #0			// reset to 0 when counter reaches 99
			CMP     R11, #59
			MOVGT   R11, #0			// reset to 0 when counter reaches 59
            ...
            LSL     R0, #24
            ORR     R3, R0
            STR     R3, [R4]        // display the number of the counter
