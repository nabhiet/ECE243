...
KEY_ISR:		PUSH     {R0-R7, LR}
				...
				LDR      R2, [R0, #0xC]  //read edge capture register
				STR		 R2, [R0, #0xC]	 // clear the interrupt
				LDR      R4, =0xFF200020 // base address of HEX3-HEX0

CHECK_KEYS:		LDR      R3, [R4]
				CMP      R2, #1
				BEQ      DISPLAY_KEY0
				CMP      R2, #2
				BEQ      DISPLAY_KEY1
...
DISPLAY_KEY0:   MOV      R5, #0b00111111
				MOV      R6, #0              // number of bits needed to shift
                LDRB     R2, [R4]            // get current HEX0 code 
                CMP      R2, R5              // check if display is already showing 0
                BEQ		 BLANK               // display blank if currently showing 0
                ORR      R3, R3, R5
                STR      R3, [R4]            // display 0
                B        KEY_ISR_END

DISPLAY_KEY1:	MOV      R5, #0b00000110
				MOV      R6, #8              // number of bits needed to shift
				LDRB     R2, [R4, #1]        // get current HEX1 code 
                CMP      R2, R5              // check if display is already showing 1
				BEQ		 BLANK               // display blank if currently showing 1
				LSL 	 R5, R6
				ORR      R3, R3, R5
                STR      R3, [R4]            // display 1
                B        KEY_ISR_END

DISPLAY_KEY2:	MOV      R5, #0b01011011
				MOV      R6, #16             // number of bits needed to shift
				LDRB     R2, [R4, #2]        // get current HEX2 code 
                CMP      R2, R5              // check if display is already showing 2
                BEQ		 BLANK               // display blank if currently showing 2
				LSL 	 R5, R6
                ORR      R3, R3, R5
                STR      R3, [R4]            // display 2
                B        KEY_ISR_END

DISPLAY_KEY3:	MOV      R5, #0b01001111
				MOV      R6, #24             // number of bits needed to shift
				LDRB     R2, [R4, #3]        // get current HEX3 code 
                CMP      R2, R5              // check if display is already showing 3
                BEQ		 BLANK               // display blank if currently showing 3
				LSL 	 R5, R6
                ORR      R3, R3, R5
                STR      R3, [R4]            // display 3
                B        KEY_ISR_END
...
BLANK:          LSL      R5, R6
				SUB      R3, R5              // subtract to place a 0 in the desired bits
				STR      R3, [R4]
				B        KEY_ISR_END
				
KEY_ISR_END:	POP      {R0-R7, LR}
				MOV      PC, LR
